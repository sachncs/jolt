...
class FlashJoLTCompressor(JoLTCompressor):
    ...
    def compress_cell(  # type: ignore[override]
        ...
        """Compress one cell using a randomized-SVD token mode.

        Identical to :meth:`JoLTCompressor.compress_cell` except that
        the randomized SVD is forced and its sketch size is bounded by
        :func:`flashjolt_cap`. The randomized SVD reports ``tail_mass``
        as a tight upper bound on the discarded spectral tail, so the
        allocator's error signal agrees with exact JoLT.

        Args:
            x: K or V tensor (``(m, T, dh)``).
            allocation: an :class:`Allocation`.

        Returns:
            A :class:`JoLTFactors` (tucker, residual, allocation).
        """
        ...
        # Honor explicit cap overrides; otherwise auto-compute from context
        # length and target ratio (sublinear in T, capped at 512).
        cap = (
...
class CapWrapper:
    ...
    def exact(self, a, rank=None):
        """Delegate to the wrapped SVD's exact path.

        Provided so callers that bypass ``__call__`` still work; FlashJoLT
        itself only triggers the ``randomise`` path.
        """
        return self.base.exact(a, rank=rank)

    def randomise(self, a, rank, *, cap=None):
        """Forward to the wrapped SVD with the FlashJoLT cap.
