...
class JointAllocator:
    ...
    def optimize(
        ...
        *,
        ...
        Algorithm (paper §4.2):
            ...
            1. For each cell, build the discrete ``(rT, rd, b)`` candidate
               grid using Eq. 1 (cost) and Eq. 2 (error).
            2. Scan a logspace of Lagrange multipliers ``λ ∈ [1e-12, 1e+6]``,
               picking the per-cell argmin of ``error + λ · cost`` at each.
               Use log-space distance to the target ratio for ranking.
            3. Refine the bracket around the best ``λ`` with bisection
               (60 outer + 40 bisection steps) until the achieved ratio
               is within the log-distance winner.
        ...
        Args:
            ...
            tau_table: optional precomputed ``τ(rT, rd)`` lookup table per
                ...
                used as a stand-in (``max(1 - rT/T, 1 - rd/d)``).
        ...
        Notes:
            ``lambda_star`` is reported for diagnostics but is not used
    ...
    def build_cell_grid(
        ...
        Cost is Eq. 1::
            ...
            cost(rT, rd, b) = (m·rT·rd + T·rT + dh·rd) · c
                            + (b/8) · m · T · dh

        Error is Eq. 2::

            error(rT, rd, b) = ε²(b) · τ(rT, rd)

        Args:
            cell: target cell.
            tau_table: optional precomputed ``τ(rT, rd)`` lookup.
    ...
    def make_result(
        ...
        Args:
            ...
            original_per_cell: original byte count per cell (for ratio).
            target_bytes: budget ``B`` the optimiser was aiming at.
...
class GreedyAllocator:
    ...
    def optimize(self, cells: Sequence[Cell]) -> AllocationResult:
        ...
        evaluate three candidate moves per cell (double the bits, raise
        feature rank by one, raise token rank by one), pick the
        largest ``Δerror / Δbytes`` ratio, and apply it. Stops when the
        budget is met or no improving move is found.
