...
class JoLTOffloadWorker:
    ...
    def __init__(
        ...
        self.pool = eviction_pool if eviction_pool is not None else {}
    ...
    def evict(self, block: Any) -> Any:
        ...
        payload = mgr.cache.payload(layer, "key")  # type: ignore[attr-defined]
        self.pool[layer] = payload
        return payload
    ...
    def restore(self, handle: Any) -> Any:
        ...
        for payload in handle:
            ...
            layer_idx = next(
                (e.layer for e in getattr(self, "meta", []) if e.kind == "value"),
                None,
            )
            v_payload = self.pool.get(layer_idx) if layer_idx is not None else None
            if v_payload is None:
                # No matching V — return K only.
                k, _ = self.compressor.decompress(handle, handle)
                out.append(k)
            else:
                k, v = self.compressor.decompress(handle, v_payload)
                out.append(k)
                out.append(v)
        return out
