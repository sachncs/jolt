...
def make_synthetic_kv(
    ...
    * V: token-rank-at-10%-error = 563, feature-rank-at-10%-error = 126.
    ...
    def make(rank_T: int, rank_d: int) -> torch.Tensor:
        """Sample a low-rank-ish tensor that mirrors real KV cache spectra.

        The core has ``1/i``-style singular-value decay (matches the paper's
        measured Mistral-7B spectra). The bases are scaled by ``1/√T``
        and ``1/√dh`` so the output's Frobenius norm is roughly ``O(1)``.
        """
...
class ReconstructionResult:
    """Per-method reconstruction stats from :func:`run_table2`.

    Attributes:
        method: name of the compressor.
        bits: residual bit-width used (None for non-quantising compressors).
        rel_err_K: relative Frobenius error on K.
        rel_err_V: relative Frobenius error on V.
        bytes_K: bytes of the K payload.
        bytes_V: bytes of the V payload.
        compression_ratio: ``(bytes_K + bytes_V) / 2`` inverse ratio; lower
